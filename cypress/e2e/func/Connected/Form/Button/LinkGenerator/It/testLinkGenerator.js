export default () => {
  it('Test button : link generator', () => {
    ...
    cy.get("input#ressource").should('be.disabled') //Disabled because ressource value is missing
    ...
    cy.get("input#ressource").should('not.be.checked')
    ...
    //Go back and fill ressource
    cy.get("button#CloseButton").click()

    //Fill email ressource
    ...
    //Check if element are disabled or not
    cy.get("input#ressource").should('not.be.disabled')
    cy.get("input#period").should('not.be.disabled')
    cy.get("input#filename").should('not.be.disabled')
    cy.get("input#extension").should('not.be.disabled')
    ...
    //Check if element not empty are checked
    cy.get("input#ressource").should('be.checked')
    cy.get("input#period").should('be.checked')
    cy.get("input#filename").should('be.checked')
    cy.get("input#extension").should('be.checked')
    cy.get("input#download").should('not.be.checked')
    ...
    cy.get('input#floating_outlined_ressource').invoke('val').then(text => ressourceValue = text);
    ...
    cy.then(() => {
      ...
      let end = dateValue[1]
      cy.assertCopiedToClipboard(`${Cypress.env("SITE_NAME")}?ressource=${ressourceValue}&start=${start}&end=${end}&filename=${filenameValue}&extension=${extensionValue}&download`)
    })
