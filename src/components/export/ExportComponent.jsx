    ...
    <>
      ...
      className="flex flex-col m-auto bg-[#EEEEEE] p-10 gap-10 rounded-xl">
        ...
        <div className="flex justify-center">
          <SearchBar value={userSearch} setter={setUserSearch} placeholder={translationHandler("resource")} user={authSession?.user?.email || ""} authSession={authSession} required={true}/>
        </div>
        ...
        </div>
        {
          ...
          <div className={`grid w-full grid-cols-[auto_65%_auto] gap-2`}>
            <LinkGeneratorButton label={translationHandler("linkGenerator")} data={{resource: userSearch, start: startDate ? dayjs(startDate).format("YYYY-MM-DD") : "", end: endDate ? dayjs(endDate).format("YYYY-MM-DD") : "", filename: fileName, extension: exportExt}}/>
