...
export default ({id, value, setter, placeholder, exportedValiditySetter, type="text", pattern="", title="", autoComplete=false, borderColorClassName=false, labelColorClassName=false, required=false}) =>{
  ...
  return (
    <div className={`duration-200 transform relative text-xl block font-light w-full bg-transparent border rounded-lg appearance-none focus:outline-none focus:ring-0 ${borderColorClassName ? borderColorClassName : `focus-within:border-[#B51F1F] ${pattern ? isValid ? "border-[#FF0000]" : "border-gray-500" : value ? "border-[#FF0000]" : "border-gray-500"}`}`}>
      ...
      {/* <svg id={`${id}-remover`} className={`h-5 w-5 absolute top-3.5 right-3.5 text-gray-400 ${value ? "" : "hidden"} animate-[ping_.2s_ease-in-out_forwards_reverse] hover:cursor-pointer`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" onClick={()=>handleDeleteEntry()} tabIndex={0} onKeyDown={(e)=> {closingHandler(e) ? (handleDeleteEntry()) : ("")}}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"></path>
      </svg> */}
      <button type="button" id={`${id}-remover`} className={`h-5 w-5 absolute top-3.5 right-3 ${value ? "" : "hidden"}`} onClick={()=>handleDeleteEntry()} tabIndex={0} onKeyDown={(e)=> {closingHandler(e) ? (handleDeleteEntry()) : ("")}}>
        <svg className={`text-gray-400 ${value ? "" : "hidden"} animate-[ping_.2s_ease-in-out_forwards_reverse] hover:cursor-pointer`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"></path>
        </svg>
      </button>
    </div>
